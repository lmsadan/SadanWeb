<template>
  <div class="wrapper activities">
    ...
    <div class="img-text">
      <div class="left-img">
        <img class="sizeImg" :src="item.image" alt="">
      </div>
      <div class="right-txt">
        <p>开始时间： {{ item.starttime | formatDate }}</p>
        <p>结束时间： {{ item.endtime | formatDate }}</p>
        <p>举办地点： {{ item.address }}</p>
        <p>主办方：{{ item.sponsor }}</p>
        <p>报名截止： {{ item.enrolltime }}</p>
        <div class="join">
          <button class="sui-btn btn-danger">
            立即报名
          </button>
    ...
    <div class="simple-text">
      ...
      <div class="right-intro">
        ...
        <div class="content-item">
          ...
          <div class="social-share" data-sites="weibo,wechat,qq" :data-title="item.name" />
        </div>
      </div>
    </div>
  </div>
