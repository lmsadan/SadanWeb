<template>
  <div class="wrapper tag-item">
    <div class="job-intro">
      <div class="left-img">
        <img :src="enterprise_item.logo" class="sizeImg-id" alt="">
      </div>
      ...
      <div class="right-tool">
        <p class="throw">
          <button class="sui-btn btn-throw">
            投简历
          </button>
        </p>
        <button class="sui-btn btn-collect">
          收藏
        </button>
        <span>100收藏</span>
        <span>291浏览</span>
...
export default {
  asyncData ({ params }) {
    return recruitApi.findById(params.id).then((res1) => {
      return enterpriseApi.findById(res1.data.data.eid).then((res2) => {
        return {
